...
import { Video, VideoError } from '../../services/video';
import { searchVideos, getSearchSuggestions, trackRecentSearch } from '../../services/video';
...
export const SearchScreen = () => {
  ...
  const handleQueryChange = useCallback(async (text: string) => {
    ...
    try {
      const suggestionsResult = await getSearchSuggestions(text);
    ...
    try {
      if (searchQuery) {
        ...
        if ('code' in response) {
          ...
          if (response.retry && retryCount < MAX_RETRIES) {
          ...
          setError(ERROR_MESSAGES[response.code] || response.message);
          setSearchResults([]);
        } else {
          ...
          try {
            await trackRecentSearch(searchQuery);
          } catch (trackError) {
            // Don't show tracking errors to user
